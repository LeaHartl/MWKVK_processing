...
def writetofile(dat, what, gl, forheader):
    if what == 'annual':
        header1 = '# Mass Balance; '+forheader+' annual fixed date'
        fn = gl+'/'+gl+'_annual_pits_stakes_fixeddate.csv'
    if what == 'seasonal':
        header1 = '# Mass Balance; '+forheader+' winter fixed date'
        fn = gl+'/'+gl+'_winter_pits_fixeddate.csv'
    if what == 'period':
        header1 = '# Mass Balance; '+forheader+' intermediate'
        fn = gl+'/'+gl+'_intermediate_pits_stakes.csv'
    if what == 'cumul':
        header1 = '# Mass Balance; '+forheader+' intermediate'
        fn = gl+'/'+gl+'_intermediate_stakes_cumul.csv'
...
# -----------
# VK:
yrsVK = np.arange(2012, 2023)
fnameVK = 'VK/Pegeldaten_gesammelt_v5.xlsx'
# manually fixed date format in excel file (if two days, alway used second date)
fpitsVK = 'VK/Schachtdaten-gesammelt_VEK_LH.xlsx'
# -----------
# MWK:
...
for gl in ['MWK', 'VK']:
    ...
    # concatenate inetrmediate stakes and pits:
    ...
    writetofile(data_annual_u, 'annual', gl, forheader)
    writetofile(data_winter_u, 'seasonal', gl, forheader)
    writetofile(data_int_u, 'period', gl, forheader)
    writetofile(stks_int_cu, 'cumul', gl, forheader)
